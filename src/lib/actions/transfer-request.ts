...
import { content, DEFAULT_LOCALE, LOCALES } from '@/content';
import {
  buildRequestLine,
  buildRequestMessage,
...
/**
 * Die Sprache des Absenders.
 *
 * Sie kommt aus dem Formular, wird aber nicht geglaubt: eine Server-Action ist
 * ein öffentlicher Endpunkt, und alles, was von dort kommt, ist eine
 * Behauptung. Ein unbekannter Wert fällt still auf Deutsch zurück — daran
 * hängen nur Beschriftungen, nichts Sicherheitsrelevantes.
 */
...
export async function submitTransferRequest(input: unknown): Promise<RequestResult> {
  const locale = localeSchema.parse(
    input && typeof input === 'object' ? (input as { locale?: unknown }).locale : null,
  );
  const t = content(locale);

  const parsed = transferRequestSchema(locale).safeParse(input);
  ...
  if (!parsed.success) {
    ...
    return { ok: false, error: t.errors.checkEntries, fieldErrors };
  }
  ...
  const body = buildRequestMessage(data, locale);

  const subject = `${t.notification.subject}: ${data.pickupLocation} → ${data.dropoffLocation}`;
  ...
  try {
    ...
    */
    const [email, whatsapp] = await Promise.allSettled([
      ...
      sendWhatsAppNotification(buildRequestLine(data, locale)),
    ]);
    ...
    */
    if (!emailDelivered && !whatsappDelivered) {
      return { ok: false, error: t.errors.notDelivered };
    }
