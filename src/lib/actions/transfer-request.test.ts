...
const { submitTransferRequest } = await import('./transfer-request');
...
describe('submitTransferRequest', () => {
  ...
  it('antwortet in der Sprache, in der gefragt wurde', async () => {
    const german = await submitTransferRequest({
      ...valid,
      locale: 'de',
      privacyAccepted: false,
    });
    const english = await submitTransferRequest({
      ...valid,
      locale: 'en',
      privacyAccepted: false,
    });

    expect(german.ok === false && german.error).toMatch(/Bitte prüfen/);
    expect(english.ok === false && english.error).toMatch(/check your entries/i);
  });

  it('schreibt die englische Anfrage auch englisch an den Betrieb', async () => {
    await submitTransferRequest({ ...valid, locale: 'en' });

    const message = send.mock.calls[0][0];
    expect(message.subject.startsWith('Transfer enquiry:')).toBe(true);
    expect(message.text).toContain('Pick-up: Frankfurt am Main');
    expect(message.text).not.toContain('Abholort');
  });

  it('fällt bei einer unbekannten Sprachangabe auf Deutsch zurück', () => {
    // Eine Server-Action ist ein öffentlicher Endpunkt: was von dort kommt, ist
    // eine Behauptung. Ein erfundener Wert darf sie nicht scheitern lassen.
    return expect(submitTransferRequest({ ...valid, locale: 'klingon' })).resolves.toEqual({
      ok: true,
    });
  });
});
