...
/**
 * Die Startseite — einmal gebaut, in beiden Sprachen dieselbe.
 *
 * Sie liegt als Komponente hier und nicht als Seite unter `app/`, weil es sie
 * zweimal gibt: auf `/` und auf `/en`. Zwei Dateien mit demselben Aufbau
 * würden auseinanderlaufen, sobald jemand einen Abschnitt ändert und die
 * andere Sprache vergisst — und das fällt niemandem auf, der nur eine der
 * beiden Fassungen liest.
 */
...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  const whatsapp = whatsappRequestLink(t.whatsappOpener);
  ...
  return (
    <>
        ...
        <div className="container relative z-10 grid gap-14 [&>*]:min-w-0 pb-20 pt-16 sm:pt-20 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:gap-16 lg:pb-32 lg:pt-28">
          <div>
            ...
            <h1 className="mt-6 text-balance text-[clamp(2rem,8.5vw,2.5rem)] font-semibold leading-[1.05] tracking-tight hyphens-auto sm:text-6xl lg:text-[4.25rem]">
              {home.headlineTop}
              <span className="mt-2 block text-primary">{home.headlineAccent}</span>
            </h1>
          ...
          <div className="panel bg-card/60 p-7 backdrop-blur-sm sm:p-8">
            <dl className="divide-y divide-border">
              <div className="pb-5">
                <dt className="eyebrow">{home.factsArea}</dt>
                <dd className="mt-2 text-lg font-medium">{t.company.serviceArea}</dd>
              </div>
              <div className="py-5">
                <dt className="eyebrow">{home.factsVehicles}</dt>
                <dd className="mt-2 text-lg font-medium leading-snug">{home.factsVehiclesValue}</dd>
              </div>
              <div className="pt-5">
                <dt className="eyebrow">{home.factsContact}</dt>
      ...
      {/* ================================================================
          B — Vertrauensleiste
          ================================================================ */}
      <section aria-label={home.trustLabel} className="border-y border-border bg-card/40">
      ...
      >
        <div className="container grid gap-14 [&>*]:min-w-0 lg:grid-cols-2 lg:items-start lg:gap-20">
          <div>
            <SectionHeading eyebrow={home.premiumEyebrow} title={home.premiumTitle} />
            <div className="mt-6 space-y-5 text-base leading-relaxed text-muted-foreground">
              {home.premiumParagraphs.map((paragraph) => (
                <p key={paragraph}>{paragraph}</p>
              ))}
            </div>

            {/* Versicherung & Absicherung — bewusst neutral. Der Text steht in
                den Inhalten und wird ausgetauscht, sobald eine Police
                vorliegt. Bis dahin steht hier keine Zusage. */}
            <div className="panel mt-10 p-7">
              <div className="flex items-start gap-3.5">
                <ShieldCheck className="mt-0.5 size-5 shrink-0 text-primary" aria-hidden />
                <div>
                  <h3 className="text-base font-semibold">{home.insuranceTitle}</h3>
                  <p className="mt-2.5 text-sm leading-relaxed text-muted-foreground">
                    {t.company.insuranceText}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="panel p-8 sm:p-10">
            <h3 className="eyebrow">{home.documentationTitle}</h3>
            <ul className="mt-7 space-y-4">
              {t.documentation.map((item) => (
      ...
      >
        <div className="container grid gap-14 [&>*]:min-w-0 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
          <div className="lg:sticky lg:top-28 lg:self-start">
            <SectionHeading
              eyebrow={home.requestEyebrow}
              title={home.requestTitle}
              lead={home.requestLead}
            />

            <h3 className="eyebrow mt-12">{home.priceFactorsTitle}</h3>
            <ul className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              {home.priceFactors.map((factor) => (
      ...
      <section id="unternehmen" className="scroll-mt-24 py-24 lg:py-32">
        <div className="container grid gap-14 [&>*]:min-w-0 lg:grid-cols-2 lg:items-start lg:gap-20">
          <div>
            <SectionHeading
              eyebrow={home.businessEyebrow}
              title={home.businessTitle}
              lead={home.businessLead}
            />

            <Link
              href={`${request}?kunde=gewerblich`}
          ...
          <div className="panel p-8 sm:p-10">
            <div className="flex items-center gap-3">
              ...
              <h3 className="eyebrow">{home.businessBenefitsTitle}</h3>
            </div>
            <ul className="mt-7 space-y-4">
              {t.businessBenefits.map((benefit) => (
      ...
      <section className="border-y border-border bg-card/40 py-24 lg:py-32">
        <div className="container">
          <SectionHeading eyebrow={home.reasonsEyebrow} title={home.reasonsTitle} />
      ...
      <section id="faq" className="scroll-mt-24 py-24 lg:py-32">
        <div className="container">
          <SectionHeading eyebrow={home.faqEyebrow} title={home.faqTitle} lead={home.faqLead} />
